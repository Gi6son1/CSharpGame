...
{
    float BulletForwardForce = 35f;  //this will hold the bullet forward force
    Transform player; //will hold the transform component of the player
    public GameObject BossBullet; //will call the boss bullet gameobject
    public GameObject BulletEmitter; //will call the bullet emitter object
    public float offset; //will hold the value for the angle offset (so that the gun faces directly at the player)
    Vector2 direction; //will hold the direction from enemy to player
    // Start is called before the first frame update
    void Start()
    {
        InvokeRepeating("BossShoot", 1f, 1.5f); //calls shoot function every 1.5 seconds, starting 1 second after being loaded in
    ...
    void Update()
    {
        Vector3 vectorToTarget = player.position - transform.position; //finds vector from boss to player
        float angle = (Mathf.Atan2(vectorToTarget.y, vectorToTarget.x) * Mathf.Rad2Deg) + offset; //calculates the angle
    ...
    void BossShoot()
    {
        ...
        TemporaryBulletHandler = Instantiate(BossBullet, BulletEmitter.transform.position, transform.rotation) as GameObject;
        ...
        Destroy(TemporaryBulletHandler, 4f); //destroys the bullet after 4 seconds, this means the computer won't be slowed down due to lots of objects in the game
    }
}
